...
class Genetic:
	...
	def recombination(self, parent_1, parent_2, p_cross, children):
		temp = rand()
		if temp < p_cross:
	...
	def crossover(self, parent_1, parent_2, p_cross):
		children = [parent_1.copy(), parent_2.copy()]
		self.recombination(parent_1, parent_2, p_cross, children)
		return [children[0], children[1]]

	def exchange_genome(self, genome, i):
		genome[i] = 1 - genome[i]
		
	def mutation(self, genome, p_mut):
		for i in range(len(genome)):
			temp = rand()
			if temp < p_mut:
				self.exchange_genome(genome, i)
				
	def genetic_algorithm(self, fitness_function, bounds, l, t, n, p_cross, p_mut):
		...
		for gen in range(t):
			...
			for i in range(0, n, 2):
				...
				for c in self.crossover(p1, p2, p_cross):
					self.mutation(c, p_mut)
					children.append(c)
			pop = children
	...
	def __init__(self):
		...
		t = 1000
		p_cross = (0.50 + yz /100)
		p_mut = (0.003 + yz/10000)
		bounds = [[-5.0, 5.0], [-5.0, 5.0]]
		best, score = self.genetic_algorithm(self.fitness_function, bounds, l, t, n, p_cross, p_mut)
		print('Done!')
		decoded = self.decode(bounds, l, best)
